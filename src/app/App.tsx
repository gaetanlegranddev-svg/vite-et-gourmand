...
const THEME_STYLES: Record<Theme,string> = { "Noël":"bg-primary text-primary-foreground","Pâques":"bg-[#4A7C59] text-white","classique":"bg-foreground text-background","événement":"bg-accent text-accent-foreground" };
...
function AuthModal({ onClose, onLogin, initialTab="login" }: { onClose:()=>void; onLogin:(u:AuthUser)=>void; initialTab?: AuthTab }) {
...
function LegalModal({ type, onClose }: { type:"mentions"|"cgv"; onClose:()=>void }) {
  const trapRef = useFocusTrap(true);
  return (
...
function AllMenusView({ menus, dishes, onDetail }: { menus:MenuData[]; dishes:Dish[]; onDetail:(id:string)=>void }) {
      ...
      {showFilters && (
        <div id="filters-panel" className="bg-card border border-border p-6 mb-8 space-y-6">
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
            ...
            <div className="space-y-2"><label htmlFor="filter-people" className="text-xs tracking-widest uppercase text-muted-foreground">Taille de mon groupe</label><input id="filter-people" type="number" min={1} placeholder="Ex : 6" value={filters.minPeople} onChange={e=>setFilters(f=>({...f,minPeople:e.target.value}))} className={ic}/><p className="text-[10px] text-muted-foreground">Affiche les menus accessibles pour ce groupe.</p></div>
          </div>
...
function Footer({ hours, onLegal }: { hours:HourSlot[]; onLegal:(t:"mentions"|"cgv")=>void }) {
  ...
  return (
    <footer className="bg-foreground text-background/70">
      <div className="max-w-6xl mx-auto px-6 pt-12 pb-8">
        ...
        <div className="border-t border-background/10 pt-6 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs text-background/30"><p>© 1999–2026 Vite &amp; Gourmand — Tous droits réservés</p><p>Site conforme RGPD &amp; RGAA · Données hébergées en France</p></div>
      </div>
    </footer>
  );
}
